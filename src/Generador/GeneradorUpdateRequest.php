...
class GeneradorUpdateRequest {
    ...
    public function generar(Tabla $tabla) {
        ...
        $contenidoId = '$id = intval($this->' . $tabla->getNombreTabla() . ');';

        $creadorRulesCampo = new CreadorRulesCampo($tabla, false);
        ...
        $contenidoPlantilla = str_replace('$TIPO_REQUEST$', 'Update', $contenidoPlantilla);
        $contenidoPlantilla = str_replace('$CONTENIDO_ID$', $contenidoId, $contenidoPlantilla);
        ...
        $archivo = $nombreClase . 'UpdateRequest.php';
